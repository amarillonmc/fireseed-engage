...
-- 旧有效关系没有历史花名册，只能以仍在原联盟的当前成员作最佳努力回填。 / Legacy active relations have no historical roster, so use current members of the former alliance as a best-effort backfill.
INSERT IGNORE INTO `vassal_rescue_eligibility`
  (`relation_id`, `eligible_user_id`)
SELECT vr.relation_id, am.user_id
FROM vassal_relations vr
INNER JOIN alliance_members am
  ON am.alliance_id = vr.previous_alliance_id
WHERE vr.status = 'active'
  AND am.user_id <> vr.vassal_id;

-- 已保存且不是被征服者本人的原势力领袖同样是可靠的旧资格线索。 / A saved former-force owner other than the vassal is also a reliable legacy eligibility signal.
INSERT IGNORE INTO `vassal_rescue_eligibility`
  (`relation_id`, `eligible_user_id`)
SELECT vr.relation_id, vr.previous_force_owner_id
FROM vassal_relations vr
WHERE vr.status = 'active'
  AND vr.previous_force_owner_id <> vr.vassal_id;
...
-- 只补齐缺失的后台项，保留管理员已经调整过的值。 / Add missing administration keys without overwriting values already chosen by an administrator.
